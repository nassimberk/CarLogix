...
class ExcelManager:
    ...
    def create_excel_file(self):
        ...
        ws.append([
            ...
            "Type Huile", "Fluide Dispo", "Relevé KMS (Prochaine Révision)", "Date Dernière Révision",
...
class ModernVehicleManagement:
    ...
    @staticmethod
    def validate_immatriculation(e):
        raw_value = re.sub(r'[^A-Za-z0-9]', '', e.control.value.upper())

        if len(raw_value) < 7:
            e.control.error_text = "L'immatriculation doit être sous format AB-123-CD"
            e.control.update()
            return

        letters_part1 = ''.join([c for c in raw_value[:2] if c.isalpha()])
        numeric_part = ''.join([c for c in raw_value[2:5] if c.isdigit()])
        letters_part2 = ''.join([c for c in raw_value[5:7] if c.isalpha()])

        formatted_value = f"{letters_part1}-{numeric_part}-{letters_part2}".strip('-')
    ...
    @staticmethod
    def validate_code_carte(e):
        current_value = e.control.value
        cleaned_value = re.sub(r'[^0-9]', '', current_value)[:4]

        if current_value != cleaned_value:
            e.control.value = cleaned_value
            e.control.update()

        if len(cleaned_value) > 0 and len(cleaned_value) < 4:
            e.control.error_text = "Le code doit contenir 4 chiffres"
            e.control.update()
        else:
            e.control.error_text = None
            e.control.update()
    ...
    @staticmethod
    def create_code_carte_field(width: int, initial_value: Optional[str] = None) -> ft.TextField:
        return ft.TextField(
            label="Code Carte",
            value=str(initial_value)[:4] if initial_value else None,
            width=width,
            height=ModernVehicleManagement.FIELD_HEIGHT,
            keyboard_type="number",
            on_change=ModernVehicleManagement.validate_code_carte,
    ...
    def create_stats_view(self):
        vehicles = self.excel.fetch_all_vehicles()
        if not vehicles:
            ...
            ft.Row([
                ft.Card(
                    content=ft.Container(
                        content=ft.Column([
                            #ft.Text("Kilométrage moyen", size=18, weight=ft.FontWeight.BOLD),
                            #ft.Text(f"{int(avg_kms)} km", size=15),
                        ]),
                        padding=20,
                    ),
                    width=200,
                ...
                ft.Card(
                    content=ft.Container(
                        content=ft.Column([
                            #ft.Text("Kilométrage mensuel moyen", size=18, weight=ft.FontWeight.BOLD),
                            #ft.Text(f"{int(avg_monthly_kms)} km/mois", size=15),
                        ]),
                        padding=20,
                    ),
                    width=200,
                ...
                ft.Card(
                    content=ft.Container(
                        content=ft.Column([
                            #ft.Text("Émission moyenne de CO2", size=18, weight=ft.FontWeight.BOLD),
                            #ft.Text(f"{avg_co2_emission} g/km", size=15),
                        ]),
                        padding=20,
                    ),
                    width=200,
                ...
                ft.Card(
                    content=ft.Container(
                        content=ft.Column([
                            #ft.Text("Âge moyen du parc", size=18, weight=ft.FontWeight.BOLD),
                            #ft.Text(f"{avg_age} mois", size=15),
                        ]),
                        padding=20,
                    ),
                    width=200,
                ...
                ft.Card(
                    content=ft.Container(
                        content=ft.Column([
                            ...
                            #ft.Text("\n".join([f"{site}: {percent:.2f}%" for site, percent in site_distribution.items()]), size=12),
    ...
    def show_vehicle_details(self, vehicle_id: int):
        vehicle = next((v for v in self.excel.fetch_all_vehicles() if v.id == vehicle_id), None)
        if not vehicle:
            return
    ...
    def add_vehicle(self, e):
        def save_vehicle(e):
            ...
            if all([vehicle.immatriculation, vehicle.code_carte, vehicle.societe_proprietaire, vehicle.site,
                    vehicle.utilisateur, vehicle.marque, vehicle.vehicule, vehicle.modele, vehicle.date_mise_en_service]):
                self.excel.add_vehicle(vehicle)
                dialog.open = False
                self.page.update()
                self.update_vehicles_list()
        ...
        immatriculation_field = ft.TextField(
            ...
            width=self.FIELD_WIDTH,
            height=self.FIELD_HEIGHT,
            on_change=ModernVehicleManagement.validate_immatriculation,
            hint_text="AB-123-CD"
        )
        code_carte_field = ModernVehicleManagement.create_code_carte_field(self.FIELD_WIDTH)
        ...
        releve_kms_field = ft.TextField(
            label="Relevé KMS (Prochaine Révision)",
            width=self.FIELD_WIDTH,
            height=self.FIELD_HEIGHT,
            keyboard_type="number",
            hint_text="Entrez que les chiffres",
            on_change=ModernVehicleManagement.validate_numeric,
            error_text=""
        )
    ...
    def edit_vehicle(self, vehicle_id: int):
        ...
        def save_changes(e):
            vehicle.immatriculation = immatriculation_field.value
            vehicle.code_carte = code_carte_field.value
            vehicle.societe_proprietaire = societe_proprietaire_field.value
            vehicle.site = site_field.value
            vehicle.utilisateur = utilisateur_field.value
            vehicle.marque = marque_field.value
            vehicle.vehicule = vehicule_field.value
            vehicle.modele = modele_field.value
            vehicle.date_mise_en_service = date_mise_en_service_field.data["text_field"].value
            vehicle.crit_air = crit_air_field.value
            vehicle.carburant = carburant_field.value
            vehicle.type_huile = type_huile_field.value
            vehicle.fluide_dispo = fluide_dispo_field.value
            vehicle.releve_kms = releve_kms_field.value
            vehicle.date_derniere_revision = date_derniere_revision_field.data["text_field"].value
            vehicle.derniere_revision = derniere_revision_field.value
            vehicle.periodicite_revision = periodicite_revision_field.value
            vehicle.prochain_ct = prochain_ct_field.data["text_field"].value
            vehicle.double_clef = double_clef_field.value
            vehicle.numero_scelle = numero_scelle_field.value
            vehicle.statut = statut_field.value

            if all([vehicle.immatriculation, vehicle.code_carte, vehicle.societe_proprietaire, vehicle.site,
                    vehicle.utilisateur, vehicle.marque, vehicle.vehicule, vehicle.modele, vehicle.date_mise_en_service]):
                self.excel.update_vehicle(vehicle)
                dialog.open = False
                self.page.update()
                self.update_vehicles_list()
        ...
        code_carte_field = ModernVehicleManagement.create_code_carte_field(self.FIELD_WIDTH, initial_value=vehicle.code_carte)
    ...
    def delete_vehicle(self, vehicle_id: int):
        def confirm_delete(e):
            self.excel.delete_vehicle(vehicle_id)
    ...
    def show_settings(self, e):
        settings_dialog = ft.AlertDialog(
            ...
            content=ft.Column([
                ...
                ft.ListTile(
                    leading=ft.Icon(icons.BACKUP, size=20),
    ...
    def export_to_pdf(self, df: pd.DataFrame, file_path: str):
        """
        Exporte un DataFrame Pandas au format PDF.
        """
        try:
            ...
            if any(col_width == 0 for col_width in col_widths):
                col_widths = [available_width / (len(df.columns) - 1)] * len(df.columns)
    ...
    def show_export_dialog(self, e):
        def export_data(e):
            ...
            export_format = export_format_field.value
            if not file_name:
                self.page.open(ft.SnackBar(ft.Text("Veuillez spécifier un nom de fichier."), open=True))
                return

            # Générer le DataFrame
            df = pd.DataFrame([vars(vehicle) for vehicle in self.excel.fetch_all_vehicles()])
            ...
            try:
                ...
                # Convertir le fichier en base64 pour le téléchargement
                ...
                download_button = ft.ElevatedButton(
                    text=f"Télécharger le fichier {export_format}",
                    icon=ft.icons.DOWNLOAD,
                    on_click=lambda _: self.page.launch_url(f"data:application/{export_format.lower()};base64,{base64_data}"),
                )
                export_dialog.content.controls.append(download_button)
                export_dialog.update()

                self.page.open(ft.SnackBar(ft.Text(f"Données exportées avec succès dans le fichier : {file_path}"), open=True))

            except Exception as e:
                self.page.open(ft.SnackBar(ft.Text(f"Erreur lors de l'exportation : {e}"), open=True))

        file_name_field = ft.TextField(label="Nom du fichier (sans extension)", width=300)
        export_format_field = self.create_dropdown("Choisissez le format d'exportation", ["Excel", "CSV", "PDF"])
    ...
    def calculate_maintenance_info(self):
        vehicles = self.excel.fetch_all_vehicles()  # Récupérer les véhicules
        today = datetime.today()
        maintenance_info = []

        for vehicle in vehicles:
            try:
                # Conversion des données kilométriques
                ...
                # Gestion de la date de la dernière révision
                ...
                # Déterminer la couleur de l'état d'entretien
                if 0 <= kms_difference <= 1000 or 0 < days_remaining <= 45:
                    status_color = colors.ORANGE
                elif kms_difference < 0 or days_remaining < 0:
                    status_color = colors.RED
                else:
                    continue  # Aucun avertissement nécessaire pour ce véhicule
            ...
            except (ValueError, AttributeError):  # Gérer les données manquantes ou incorrectes
                continue
    ...
    def show_maintenance_alerts(self):
        ...
        if not maintenance_info:
            ...
            # Créer une carte pour chaque véhicule nécessitant un entretien
            for info in maintenance_info:
                vehicle = info["vehicle"]
                prochaine_revision_kms = info["prochaine_revision_kms"]  # Extraction ici
    ...
    def calculate_ct_info(self):
        ...
        for vehicle in vehicles:
            if vehicle.prochain_ct:
                try:
                    # Convertir la date du prochain contrôle technique
                    ...
                    # Ignorer les véhicules avec une date non valide
                    continue
                ...
                # Ajouter à la liste uniquement si moins de 2 mois restants ou périmé
                if difference <= 60:
                    status_message = (
    ...
    def show_ct_alerts(self):
        ...
        if not ct_info:
            # Ajouter un message si tous les véhicules sont à jour
            alert_view.controls.append(
                ft.Card(
                    content=ft.Container(
                        content=ft.Text(
            ...
            # Ajouter les alertes pour les véhicules concernés
